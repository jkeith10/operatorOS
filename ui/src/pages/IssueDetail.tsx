...
import type { ActivityEvent } from "@operatoros/shared";
import type { Agent, FeedbackVote, Issue, IssueAttachment, IssueComment } from "@operatoros/shared";
...
export function IssueDetail() {
  ...
  const [highlightAttachmentId, setHighlightAttachmentId] = useState<string | null>(null);
  const [galleryOpen, setGalleryOpen] = useState(false);
  const [galleryIndex, setGalleryIndex] = useState(0);
  const [optimisticComments, setOptimisticComments] = useState<OptimisticIssueComment[]>([]);
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const lastMarkedReadIssueIdRef = useRef<string | null>(null);
  const attachmentHashScrollRef = useRef(false);
  ...
  useEffect(() => {
    attachmentHashScrollRef.current = false;
    setHighlightAttachmentId(null);
  }, [issueId, location.hash]);

  useEffect(() => {
    const hash = location.hash;
    if (!issueId || !hash.startsWith("#attachment-")) return;
    const targetId = decodeURIComponent(hash.slice("#attachment-".length));
    const list = attachments ?? [];
    if (!list.some((a) => a.id === targetId) || attachmentHashScrollRef.current) return;
    const el = document.getElementById(`attachment-${targetId}`);
    if (!el) return;
    attachmentHashScrollRef.current = true;
    el.scrollIntoView({ behavior: "smooth", block: "center" });
    setHighlightAttachmentId(targetId);
    const timer = window.setTimeout(() => {
      setHighlightAttachmentId((current) => (current === targetId ? null : current));
    }, 3000);
    return () => window.clearTimeout(timer);
  }, [attachments, issueId, location.hash]);
        ...
        <div className="space-y-2">
          {attachmentList.map((attachment) => (
            <div
              ...
              className={cn(
                "border border-border rounded-md p-2 scroll-mt-6 transition-colors",
                highlightAttachmentId === attachment.id && "border-primary/50 bg-primary/5 ring-1 ring-primary/30",
              )}
            >
              <div className="flex items-center justify-between gap-2">
                <a
