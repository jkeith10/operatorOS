...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm operatoros company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Paperclip](https://paperclip.ing) for upstream documentation; OperatorOS uses the same portable company package conventions.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from OperatorOS on ${new Date().toISOString().split("T")[0]} (compatible with Paperclip-format company packages).`);
  lines.push("");
