...
- **Recent deliverables** — latest issue-linked documents and file attachments (newest first), with links back to the task
...
Returns agent counts by status, task counts by status, cost summaries, budget rollup fields, and `recentDeliverables` (issue documents and attachments merged by recency).
