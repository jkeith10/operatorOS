...
const DATA_DIR_OPTION_HELP =
  "Instance data directory root (isolates state from the default ~/.operatoros home; legacy: ~/.paperclip)";

const invokedName = (() => {
  try {
    const base = process.argv[1]?.split(/[/\\]/).pop() ?? "";
    return base.replace(/\.[cm]?js$/i, "") || "operatoros";
  } catch {
    return "operatoros";
  }
})();
...
program
  .name(invokedName)
  .description("OperatorOS CLI — setup, diagnose, and configure your instance (forked from Paperclip)")
  .version(cliVersion);
