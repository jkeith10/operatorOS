...
describe("applyDataDirOverride", () => {
  ...
  it("sets OPERATOROS_HOME, PAPERCLIP_HOME, and isolated default config/context paths", () => {
    ...
    expect(process.env.OPERATOROS_HOME).toBe(expectedHome);
  ...
  it("only applies defaults for options supported by the command", () => {
    ...
    expect(process.env.OPERATOROS_HOME).toBe(path.resolve("/tmp/paperclip-alt"));
