...
if (!companyE2eHasServerDist) {
  console.warn(
    "Skipping company import/export e2e: server/dist/index.js not found. Run `pnpm build` (or `pnpm --filter @operatoros/server build`) before `pnpm test:run`.",
  );
}
...
/** Matches `ensureLocalSecretsKeyFile` so `paperclipai doctor` does not need a repair pass. */
function ensureTestSecretsMasterKey(tempRoot: string) {
  const keyPath = path.join(tempRoot, "secrets", "master.key");
...
function createServerEnv(configPath: string, port: number, connectionString: string) {
  ...
  // Avoid doctor repair + guarantee server auth env when parent strips PAPERCLIP_* (e.g. dev machine with ~/.paperclip).
  env.PAPERCLIP_AGENT_JWT_SECRET =
    process.env.PAPERCLIP_AGENT_JWT_SECRET?.trim() ||
    "paperclip-cli-e2e-agent-jwt-secret-32chars-min";
...
async function waitForServer(
  ...
  const deadlineMs = Number(process.env.PAPERCLIP_E2E_SERVER_WAIT_MS) || 180_000;
...
describeCompanyImportExportE2e("paperclipai company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
