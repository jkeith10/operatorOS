...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (!configExists(configPath)) {
    ...
    if (nonInteractive && !opts.yes) {
      ...
      p.log.message(
        `Run ${pc.cyan("paperclipai onboard --yes")} once, or retry with ${pc.cyan("paperclipai run --yes")}.`,
      );
      process.exit(1);
    }
    ...
    p.log.step(
      opts.yes
        ? "No config found. Creating quickstart config (--yes)..."
        : "No config found. Starting onboarding...",
    );
    await onboard({ config: configPath, invokedByRun: true, yes: opts.yes === true });
  }
...
function maybeEnableUiDevMiddleware(entrypoint: string): void {
  ...
  if (normalized.endsWith("/server/src/index.ts") || normalized.endsWith("@operatoros/server/src/index.ts")) {
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  // Production mode: import the published @operatoros/server package
  try {
    const mod = await import("@operatoros/server");
    return await startServerFromModule(mod, "@operatoros/server");
  } catch (err) {
    const missingSpecifier = getMissingModuleSpecifier(err);
    const missingServerEntrypoint = !missingSpecifier || missingSpecifier === "@operatoros/server";
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a Paperclip server entrypoint.\n` +
          `Tried: ${devEntry}, @operatoros/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
