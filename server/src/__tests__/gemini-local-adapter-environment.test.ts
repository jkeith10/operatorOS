...
import { testEnvironment } from "@operatoros/adapter-gemini-local/server";
import { writeFakeCliCommand } from "./helpers/fake-cli-command.js";

const FAKE_GEMINI_PROBE_SCRIPT = `const fs = require("node:fs");
...
const FAKE_GEMINI_QUOTA_SCRIPT = `if (process.argv.includes("--help")) {
  process.exit(0);
}
...
describe("gemini_local environment diagnostics", () => {
  ...
  it("passes model and yolo flags to the hello probe", async () => {
    ...
    await writeFakeCliCommand(binDir, "gemini", FAKE_GEMINI_PROBE_SCRIPT);
  ...
  it("classifies quota exhaustion as a quota warning instead of a generic failure", async () => {
    ...
    const cwd = path.join(root, "workspace");
    await fs.mkdir(binDir, { recursive: true });
    await writeFakeCliCommand(binDir, "gemini", FAKE_GEMINI_QUOTA_SCRIPT);
