...
import type { AdapterRuntimeServiceReport } from "@operatoros/adapter-utils";
import type { Db } from "@operatoros/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@operatoros/db";
...
function isWindowsCmdShell(shell: string): boolean {
  const base = path.basename(shell).toLowerCase();
  return base === "cmd.exe" || base === "cmd";
}

/** Spawn arguments for running a shell `command` string (POSIX `-c` / `-lc`, Windows `cmd /c`). */
function shellInvokeArgs(command: string, loginShell: boolean): string[] {
  const shell = resolveShell();
  if (process.platform === "win32" && isWindowsCmdShell(shell)) {
    return ["/d", "/s", "/c", command];
  }
  return [loginShell ? "-lc" : "-c", command];
}
...
async function startLocalRuntimeService(input: {
  ...
  const shell = resolveShell();
  const child = spawn(shell, shellInvokeArgs(command, true), {
    cwd: serviceCwd,
    env,
