...
### Manual first publish for `@operatoros/ui`
...
npm view @operatoros/ui version

# make sure the dist payload is fresh
pnpm --filter @operatoros/ui build
...
- If `npm view @operatoros/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
