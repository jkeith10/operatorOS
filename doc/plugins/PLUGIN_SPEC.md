...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@operatoros/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@operatoros/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@operatoros/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@operatoros/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@operatoros/plugin-sdk/ui`
...
The `@operatoros/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
### 27.1 `@operatoros/plugin-test-harness`
...
```ts
import { createTestHarness } from "@operatoros/plugin-test-harness";
...
- `@operatoros/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@operatoros/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@operatoros/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@operatoros/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@operatoros/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@operatoros/plugin-sdk/ui`
...
- `@operatoros/plugin-test-harness`
