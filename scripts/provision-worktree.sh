...
run_paperclipai_command() {
  local command_args=("$@")
  if command -v pnpm >/dev/null 2>&1 && pnpm operatoros --help >/dev/null 2>&1; then
    pnpm operatoros "${command_args[@]}"
    return $?
  fi
...
async function main() {
  ...
  const envLines = [
    "OPERATOROS_HOME=" + JSON.stringify(worktreeHome),
